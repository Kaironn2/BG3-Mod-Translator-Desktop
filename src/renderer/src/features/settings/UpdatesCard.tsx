...
export function UpdatesCard(): React.JSX.Element {
  const { state, checking, check, install } = useUpdater()
  const { t } = useAppTranslation('updater')
  // Background checks keep the manual button enabled; only the renderer's own
  // check (spinner on the button itself) or an install flow blocks it.
  const busy =
    state.status === 'backing-up' || state.status === 'downloading' || state.status === 'installing'
  const canInstall =
    (state.status === 'available' || state.status === 'ready') && state.channel === 'installed'
  const canCheck = state.channel === 'installed' && !busy && !checking
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-md border border-[#1f2329] bg-[#0f1114] px-4 py-2 text-sm text-neutral-300 transition-colors hover:border-[#252a32] hover:text-neutral-100 disabled:cursor-not-allowed disabled:opacity-50"
          >
...
function VersionField({ label, value }: { label: string; value: string }): React.JSX.Element {
  return (
    <div className="rounded-md border border-[#1f2329] bg-[#0f1114] px-3 py-2.5">
