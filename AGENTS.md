...
Live DB path: `%APPDATA%/Icosa/icosa.db` for the installed app; portable uses `<exe-dir>/data/icosa.db`. Windows auto-update (NSIS + `electron-updater`) replaces app code only. Never commit `.env`, `data/`, or `scripts/bench-results/` (those can contain API keys and a full dictionary copy).
